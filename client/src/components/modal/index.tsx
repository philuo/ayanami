...
export function OModal(p: Props) {
  ...
  const combineClass = p.class ? `o_modal ${p.class}` : 'o_modal';
    ...
    <Teleport when={mounted()}>
      <div class={combineClass}>
        <Transition name="fade-in" appear={appear()}>
          {show() && d.showMask && <div class="o_modal_mask" onClick={() => d.maskClosable && maskClose()} />}
        </Transition>
        ...
        >
          {show() && <div ref={modelContentRef} class="o_modal_content" use:draggable={d.draggable}>{d.children}</div>}
        </Transition>
      </div>
    </Teleport>
  );
